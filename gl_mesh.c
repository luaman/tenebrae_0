...
void GL_EliminateTrivialDegenerateTriangles(aliashdr_t *model);
/*
================
StripLength
================
...
qboolean nearVertex(vec3_t v1, vec3_t v2) {
	vec3_t temp;
	VectorSubtract(v1,v2,temp);
	if (Length(temp) < 0.1) return true;
	return false;
}
...
/*	for (i=0 ; i<paliashdr->numtris ; i++)
		for (j=0 ; j<3 ; j++) {
			//FIXME: slow
			//vertices are reordened we should change the vertex indieces in the
			//trangle list so that they point to the right vertex in the new order.
		
			for (k=0; k<numorder; k++) {
				if (tris[i].vertindex[j] == vertexorder[k]) {
					break;	
				}	
			}
	
			tris[i].vertindex[j] = k;
		}
*/
...
void GL_MakeAliasModelDisplayLists (model_t *m, aliashdr_t *hdr)
{
	...
	int			*cmds;
	ftrivertx_t	*verts, *v, *oldverts, *vert;
	mtriangle_t *tris, *trisi;
	int			*indecies;
	fstvert_t	*texcoords;
	...
	// save the data out
	paliashdr->poseverts = paliashdr->numverts;
	...
	//PENTA: Calculate texcoords for triangles (this duplicates some vertices that don't have different
	//texcoords for different tris)
	for (i=0; i<paliashdr->numtris ; i++) {
		for (j=0; j<3; j++) {
			s = stverts[triangles[i].vertindex[j]].s;
			t = stverts[triangles[i].vertindex[j]].t;
			...
			if (!triangles[i].facesfront && stverts[triangles[i].vertindex[j]].onseam) {
				int newindex = paliashdr->poseverts;

				//Duplicate it in all poses
				for (k=0 ; k<paliashdr->numposes ; k++) {
					poseverts[k][newindex] = poseverts[k][triangles[i].vertindex[j]];
				}
				...
				//Adapt index pointer
				triangles[i].vertindex[j] = paliashdr->poseverts;
	...
	//PENTA: Calculate floating point texcoords for this
	texcoords = Hunk_Alloc (paliashdr->poseverts * sizeof(fstvert_t));//3 coords per tris
	paliashdr->texcoords = (byte *)texcoords - (byte *)paliashdr;
	for (i=0 ; i<paliashdr->poseverts ; i++) {
		texcoords[i].s = (stverts[i].s-0.5)  / pheader->skinwidth;
		texcoords[i].t = (stverts[i].t-0.5)  / pheader->skinheight;
	}

	//PENTA: copy verts from the load-buffer to our model structure
	oldverts = verts = Hunk_Alloc (paliashdr->numposes * paliashdr->poseverts * sizeof(ftrivertx_t) );
	paliashdr->posedata = (byte *)verts - (byte *)paliashdr;
	for (i=0 ; i<paliashdr->numposes ; i++)
		for (j=0 ; j<paliashdr->poseverts ; j++) {
			vert = verts++;
			vert->v[0] = poseverts[i][j].v[0] * paliashdr->scale[0] + paliashdr->scale_origin[0];
			vert->v[1] = poseverts[i][j].v[1] * paliashdr->scale[1] + paliashdr->scale_origin[1];
			vert->v[2] = poseverts[i][j].v[2] * paliashdr->scale[2] + paliashdr->scale_origin[2];
			vert->lightnormalindex = poseverts[i][j].lightnormalindex;
		}
	verts = oldverts;
	...
	//PENTA: Calculate plane eq's for triangles (shadow volumes)
	planes = Hunk_Alloc (paliashdr->numtris * paliashdr->numposes * sizeof(plane_t));
	paliashdr->planes = (byte *)planes - (byte *)paliashdr;
	for (i=0; i<paliashdr->numposes; i++) {
		for (j=0; j<paliashdr->numtris ; j++) {

			//copy triangle's vertices to our temp triangle structure
			for (k=0; k<3; k++) {
				v = &verts[i*paliashdr->poseverts + tris[j].vertindex[k]];
				for (l=0; l<3; l++)
					triangle[k][l] = v->v[l];		
			...
			VectorSubtract(triangle[2], triangle[1], v2);
			CrossProduct(v2,v1, normal);		
			VectorScale(normal, 1/Length(normal), planes[i*paliashdr->numtris+j].normal);
			
			//distance of plane eq
			planes[i*paliashdr->numtris+j].dist = DotProduct(triangle[0],planes[i*paliashdr->numtris+j].normal);
		}
	}
